...
describe("SourcesTransform", () => {
	it("should transform", async () => {
		...
		const files = (await fg("**/*", { onlyFiles: true, ignore })).map(
			f => ({
				path: f,
		...
		assert(
			await p.hasFile(
				path.join("lib", "transform", "sourcesTransform.js"),
			),
		);
		assert(
			await p.hasFile(
				path.join("lib", "transform", "sourcesTransform.d.ts"),
			),
		);
		assert(
			await p.hasFile(
				path.join("lib", "transform", "sourcesTransform.d.ts.map"),
			),
		);
		assert(
			await p.hasFile(
				path.join("lib", "transform", "sourcesTransform.js.map"),
			),
		);
		assert(
			await p.hasFile(
				path.join("src", "lib", "transform", "sourcesTransform.ts"),
			),
		);
		assert(
			!(await p.hasFile(
				path.join("lib", "transform", "sourcesTransform.ts"),
			)),
		);
