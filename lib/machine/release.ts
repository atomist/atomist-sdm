...
export function releaseOrPreRelease(
	version: string,
	gi: GoalInvocation,
): string {
 ...
 */
export function addBranchPreRelease(
	baseVersion: string,
	goalEvent: SdmGoalEvent,
): string {
	const branch = goalEvent.branch;
	const branchSuffix =
		branch === goalEvent.push.repo.defaultBranch
			? ""
	...
	return async (gi: GoalInvocation): Promise<ExecuteGoalResult> => {
		...
		return configuration.sdm.projectLoader.doWithProject(
			...
			async p => {
				...
				if (
					!(gi.goalEvent.push.after.tags || []).some(
						t => t.name === versionRelease,
					)
				) {
					await createGitTag({
						project: p,
...
export type IncrementPatchCommand = (
	p: GitProject,
	log: ProgressLog,
	...
	return async (gi: GoalInvocation): Promise<ExecuteGoalResult> => {
		...
		return configuration.sdm.projectLoader.doWithProject(
			...
			async p => {
				...
				const log = new DelimitedWriteProgressLogDecorator(
					gi.progressLog,
					"\n",
				);
				...
				const preEls: ExecuteLogger[] = [
					...
					spawnExecuteLogger({
						cmd: { command: "git", args: ["pull", remote, branch] },
						cwd: gp.baseDir,
					}),
				];
...
[atomist:generated]`),
				...
				await loglog(
					log,
					`Incrementing version and committing for ${slug}`,
				);
...
export const IsReleaseCommit: PushTest = {
	...
	mapping: async pi => {
		...
		return (
			versionRegexp.test(pi.push.after.message) ||
			changelogRegexp.test(pi.push.after.message)
		);
	},
};
