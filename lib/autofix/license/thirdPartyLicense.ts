...
import { GitProject, NoParameters, Project } from "@atomist/sdm/lib/client";
...
export const AddThirdPartyLicense = addThirdPartyLicense(
	allSatisfied(IsNode, ToDefaultBranch, not(IsInLocalMode)),
);
...
export function addThirdPartyLicenseTransform(): CodeTransform<NoParameters> {
	return async p => {
		...
		const pj = JSON.parse(
			(await fs.readFile(path.join(cwd, "package.json"))).toString(),
		);
		...
		_.forEach(json, (v, k) => {
			...
			licenses.forEach(l => {
				...
				if (Object.prototype.hasOwnProperty.call(grouped, license)) {
					grouped[license] = [
						...grouped[license],
						{
							...v,
		...
		for (const l in counts) {
			if (Object.prototype.hasOwnProperty.call(counts, l)) {
				const anchor = l
					.toLocaleLowerCase()
					.replace(/ /g, "-")
					.replace(/\./g, "")
...
\`${pj.name}\` is licensed under ${spdx[pj.license].name} - [${
					spdx[pj.license].url
			  }](${spdx[pj.license].url}).`
			: "";
